...
// copied from GS4Mv11RS
// splits regular leaves
// also cracks irregulars moving to the end
...
bool RTREE_QUAL::InsertRectRecGradually(float* gs_tba_mins, float* gs_tba_maxes, int* gs_tba_ids, int gs_count, Rect gs_tba_mbr, Node *a_node, vector<pair<Node*, Branch>> *leaves_to_add_later, Rect &a_coverRect) {
    ...
    if (a_node->m_level > 0) {
        ...
        // if(a_node->isRegular()){
        //     // cout << "was regular " << endl;
        //     if(a_node->m_filled_spare_space + gs_count + a_node->m_R - a_node->m_L - a_node->m_holes <= MAXDATAPOINTS){
        ...
        //         // split the leaf
        //         a_node->m_filled_spare_space += gs_count;
        //         Node* a_newNode; Rect a_newCoverRect;
        //          #ifdef stats
        //             if(a_node->isRegular()){
        ...
        //         #endif
        //         // cout << "Spare count " << a_node->m_filled_spare_space << " gs_count " << gs_count << endl;
        //         SplitLeaf(a_node, &a_newNode, a_coverRect, a_newCoverRect);
        //         leaves_to_add_later->push_back(make_pair(a_node->m_parent, Branch(a_newNode, a_newCoverRect, -1)));
...
void RTREE_QUAL::ripple_lite_v2(float* this_tba_mins, float* this_tba_maxes, int* this_tba_ids, int this_tba_count, Node* this_leaf, vector<pair<Node*, Branch>> *leaves_to_add_later){
    ...
    if(this_leaf->m_R - this_leaf->m_L - this_leaf->m_holes > 2*MAXDATAPOINTS && (k < (this_leaf->m_R - this_leaf->m_L))){
        ...
        // float X = m_data_arr_mins[this_L + this_holes + rand() % (this_R - this_L - this_holes)][stochastic_crack_axis];    // split in random position
        // float X2 = m_data_arr_mins[this_L + this_holes + rand() % (this_R - this_L - this_holes)][stochastic_crack_axis];    // split in random position
        // float X3 = m_data_arr_mins[this_L + this_holes + rand() % (this_R - this_L - this_holes)][stochastic_crack_axis];    // split in random position
        ...
        if((stochastic_crack_index - this_L) < (this_R - stochastic_crack_index)){
            ...
            while((stochastic_crack_index - this_L) < this_I){
                ...
                // cout << " in while percentile " << percentile << " sample size " << SAMPLE_SIZE << " chosen pivot " << stochastic_crack_pivot << " sci " << stochastic_crack_index << " this_L " << this_L << " thi_R " << this_R << " this_holes " << this_holes << " k " << k << endl;
                // cout << "fakes: " << fake_this_L << " " << fake_this_R << endl;
                ...
                // cout << "after fakes: " << fake_this_L << " " << fake_this_R << " new prec " << percentile << endl;
        ...
        // l_rect = LeafCover(this_L + this_holes, stochastic_crack_index);
        // r_rect = LeafCover(stochastic_crack_index, this_R);
