...
int partition_on_one(int *mapping, int low, int high, int offset)
{
    // Choosing the pivot
    int folan;
    int pivot = 1;
    ...
    for (int j = low; j <= high - 1; j++) {
        ...
        if (mapping[j - offset] < pivot) {
            ...
            // if(m_data_arr_ids[i] == 296535 || m_data_arr_ids[j] == 296535){
            //     cout << "1 swapping " << i << " " << j << " " << m_data_arr_ids[i] << " " << m_data_arr_ids[j] << endl;
            // }
    ...
    // if(m_data_arr_ids[i + 1] == 296535 || m_data_arr_ids[high] == 296535){
    //     cout << "2 swapping " << i+1 << " " << high << " " << m_data_arr_ids[i+1] << " " << m_data_arr_ids[high] << endl;
    // }
...
{   
    // cout << "in partition one one" << endl;
    // Choosing the pivot
    int folan;
    int pivot = 1;
    ...
    for (int j = low; j <= high - 1; j++) {
        // cout << "looking at item " << j << " with id " << m_data_arr_ids[j] << " and mapping " << (*mapping)[j - offset] << endl; 
        // If current element is smaller than the pivot
        if ((*mapping)[j - offset] < pivot) {
 
            // Increment index of smaller element
            i++;
            // cout << "swapping " << j << " and " << i << endl;
            for(folan = 0; folan< NUMDIMS; folan++){
                swap(m_data_arr_mins[i][folan], m_data_arr_mins[j][folan]);
                swap(m_data_arr_maxes[i][folan], m_data_arr_maxes[j][folan]);
            }
    ...
    // cout << "swapping " << i + 1 << " and " << high << endl;
    for(folan = 0; folan< NUMDIMS; folan++){
        swap(m_data_arr_mins[(i + 1)][folan], m_data_arr_mins[high][folan]);
        swap(m_data_arr_maxes[(i + 1)][folan], m_data_arr_maxes[high][folan]);
    }
...
template<class DATATYPE,
        int TMAXNODES = 8, int TMINNODES = TMAXNODES / 2, int TMAXDATAPOINTS = 256, int TMINDATAPOINTS = TMAXDATAPOINTS / 2>
                ...
                public:
                    ...
                    int QueryAdaptive_v15(const float a_min[NUMDIMS], const float a_max[NUMDIMS]);
...
//                        Branch m_data_points[MAXDATAPOINTS];            ///< data points, couldnt keep them in m_branch since the limits differ
                    ...
                    struct PartitionVarsLeaf {
                        enum {
                            NOT_TAKEN = -1
                        }; // indicates that position

                        vector<int> m_partition;
                        //        std::vector<int> m_partition;
                        int m_total;
                        int m_minFill;
                        int m_count[2];
                    ...
                    typedef vector<LTA_v2_ripple> LeavesToAdd_v2_ripple;
                    ...
                    // GS
                    // bool InsertRectGradually(const Branch &a_branch, Node **a_root, int a_level);
                    ...
                    // void SplitLeaf(Node *a_node, const Branch *a_branch, Node **a_newNode, Rect &a_coverRect, Rect &a_newCoverRect);
                    void SplitLeaf(Node *a_node, Node **a_newNode, Rect &a_coverRect, Rect &a_newCoverRect);
                    // void SplitLeaf(Node *a_node, float* gs_tba_mins, float* gs_tba_maxes, int* gs_tba_ids, int gs_count, Node **a_newNode, Rect &a_coverRect, Rect &a_newCoverRect);
                    ...
                    void GetBranches(Node *a_node, PartitionVarsLeaf *a_parVars);



                    void ChoosePartition(PartitionVars *a_parVars, int a_minFill);
                    void ChoosePartition(PartitionVarsLeaf *a_parVars, int a_minFill);

                    void LoadNodes(Node *a_nodeA, Node *a_nodeB, PartitionVars *a_parVars);

                    void LoadLeaves(Node *a_nodeA, Node *a_nodeB, PartitionVars *a_parVars);
                    void LoadLeaves(Node *a_nodeA, Node *a_nodeB, PartitionVarsLeaf *a_parVars);
                    ...
                    void InitParVars(PartitionVarsLeaf *a_parVars, int a_maxRects, int a_minFill);

                    void PickSeeds(PartitionVarsLeaf *a_parVars);

                    void Classify(int a_index, int a_group, PartitionVarsLeaf *a_parVars);
                    ...
                    bool Search_2027(Node *a_node, Rect *a_rect, int &a_foundCount, LeavesToAdd_v2_ripple *all_lta, int tba_start, int tba_count, vector<overlapping_leaf_tbas> *overlapping_leaves);
                    ...
                    // like ripple v3 just puts stuff at the end, but just for one item in one leaf
...
RTREE_TEMPLATE
int RTREE_QUAL::QueryAdaptive_v15(const float *a_min, const float *a_max) {
...
bool RTREE_QUAL::InsertRectRecGradually(float* gs_tba_mins, float* gs_tba_maxes, int* gs_tba_ids, int gs_count, Rect gs_tba_mbr, Node *a_node, vector<pair<Node*, Branch>> *leaves_to_add_later, Rect &a_coverRect) {
    ...
    //         a_node_branch_index = getNodeBranchIndex(a_node);
    //         (a_node->m_parent)->m_branch[a_node_branch_index].m_rect = CombineRect(&((a_node->m_parent)->m_branch[a_node_branch_index].m_rect), &gs_tba_mbr);
    //     }
    // }

    // for(int ii = 0;ii < gs_count; ii++){
    //     if(gs_tba_ids[ii] == 500106){

    //         cout << "1 before, in insertrect_grad for item 500106, node level: " << a_node->m_level << " mbr: " << endl;
    //         if(a_node->m_parent != NULL) {
    //         for(int jj = 0; jj < NUMDIMS; jj++) cout <<  (a_node->m_parent)->m_branch[a_node_branch_index].m_rect.m_min[jj] << " " << (a_node->m_parent)->m_branch[a_node_branch_index].m_rect.m_max[jj]<< ", ";
    //         cout << endl;
    //         }
    //         else{
    ...
    //         cout << "1 after, in insertrect_grad for item 500106, node level: " << a_node->m_level << " mbr: " << endl;
    //         if(a_node->m_parent != NULL) {
    //         for(int jj = 0; jj < NUMDIMS; jj++) cout <<  (a_node->m_parent)->m_branch[a_node_branch_index].m_rect.m_min[jj] << " " << (a_node->m_parent)->m_branch[a_node_branch_index].m_rect.m_max[jj]<< ", ";
    //         cout << endl;
    //         }
    ...
    //     }
    // }
    // cout << "In gradual level " << a_node->m_level << endl;
    ...
    if (a_node->m_level > 0) {
        ...
        if(a_node->m_filled_spare_space + gs_count < SPARE_SPACE_LIMIT){
            ...
            // cout << "diffusing node: " << a_node->m_id << endl;
            // diffuse
            ...
            // ASSERT(gs_count + a_node->m_filled_spare_space < 4*SPARE_SPACE_LIMIT);
            ...
            // cout << "after mallocs tba_count " << gs_count << endl;

            Rect this_rect;
            Rect* tba_MBRs = (Rect*)malloc(a_node->m_count * sizeof(Rect));

            for(int i = 0; i < gs_count + a_node->m_filled_spare_space; i++){
                // if(a_node->m_id == 6674) cout << "checking adta " << a_node->m_spare_ids[i] << endl;
                ...
                // if(tba_to_children_counts[tba_map[i]] == 1){
                //     tba_MBRs[tba_map[i]] = this_rect;
                // }
                // else{
                //     tba_MBRs[tba_map[i]] = CombineRect(&this_rect, &(tba_MBRs[tba_map[i]]));
                // }
                // if(i >= a_node->m_filled_spare_space){
                //     if(a_node->m_parent != NULL) {
            ...
            a_node->m_filled_spare_space = 0;
            free(tba_map);
            // cout << "after free " << a_node->m_count << endl;

            for(int index = 0; index < a_node->m_count; index++){
                // cout << "recursive call child " << index << endl;
        ...
        // cout << "in insertrectrecgrad for leaf " << a_node->m_id << " whose parent is: " << a_node->m_parent->m_id << endl;
        // leaf level

        if(a_node->m_filled_spare_space + gs_count < SPARE_SPACE_LIMIT){
            // cout << "for leaf " << a_node->m_id << " there was space" << endl;
            // put them in
            // update MBR
            memcpy(a_node->m_spare_mins + a_node->m_filled_spare_space*NUMDIMS, gs_tba_mins, gs_count*NUMDIMS*sizeof(float));
            memcpy(a_node->m_spare_maxes + a_node->m_filled_spare_space*NUMDIMS, gs_tba_maxes, gs_count*NUMDIMS*sizeof(float));
        ...
        // cout << "for leaf " << a_node->m_id << " there was no space, rippling" << endl;
        ...
        // for(int gg =0; gg < gs_count + a_node->m_filled_spare_space; gg++){
        //     if(a_node->m_spare_ids[gg] == 500363){
        //         cout << "found 500363 in this leaf " << endl; 
        //     }
        // }
        ...
        auto start_time = clock();
        #endif
        // ripple_lite_v2(gs_tba_mins, gs_tba_maxes, gs_tba_ids, gs_count, a_node);
...
bool RTREE_QUAL::InsertRect(const Branch &a_branch, Node **a_root, int a_level) {
    ...
    if (InsertRectRec(a_branch, *a_root, &newNode, a_level, rect1, rect2))  // Root split
        ...
        // cout << "before add branch call" << endl;
        ...
        // cout << "before add branch call 2" << endl;
        AddBranch(&branch, newRoot, NULL);
...
void RTREE_QUAL::SplitNode(Node *a_node, const Branch *a_branch, Node **a_newNode) {
    //            ASSERT(a_node);
    //            ASSERT(a_branch);
    // cout << "in split node " << a_node->m_id << " " << a_node->m_level << " "<< endl;
    // if(a_node->m_parent != NULL){
    //     int branch_index = getNodeBranchIndex(a_node);
    //     Rect rect = (a_node->m_parent)->m_branch[branch_index].m_rect;
    //     // cout << "rect: ";
    //     // for(int k =0; k < NUMDIMS; k++) cout << rect.m_min[k] << " " << rect.m_max[k] << ", ";
    //     // cout << endl; 
    // }
    // else{
    //     cout << "is root" << endl;
...
void RTREE_QUAL::SplitNode(Node *a_node, const Branch *a_branch, Node **a_newNode, Rect &a_coverRect, Rect &a_newCoverRect) {
    ...
    // if(a_node->m_id == 6674) cout << "calling split on 6674" << endl;
    PartitionVars localVars;
    PartitionVars *parVars = &localVars;
    ...
    *a_newNode = AllocNode();
    ...
    for(int i = 0; i < a_node->m_filled_spare_space; i++){
        ...
        // if(a_node->m_spare_ids[i] == 500075){
        //     cout << "splittng item 500075, the choice was " << choice << endl;
        // }
        if(choice == 0){
            // piece 1
...
RTREE_TEMPLATE
void RTREE_QUAL::SplitLeaf(Node *a_node, Node **a_newNode, Rect &a_coverRect, Rect &a_newCoverRect) {
    //            ASSERT(a_node);
    //            ASSERT(a_branch);

    // cout << "In splitleaf " << a_node->m_id << endl;
    // Could just use local here, but member or external is faster since it is reused
    PartitionVarsLeaf localVars;
    PartitionVarsLeaf *parVars = &localVars;

    // Load all the branches into a buffer, initialize old node
    GetBranches(a_node, parVars);
    // cout << "After getBranches" << endl;

    // Find partition
    int min_fill;
    if(a_node->IsLeaf()) {min_fill = MINDATAPOINTS;} else {min_fill = MINNODES;}
    // cout << "gscount = " << gs_count << " maxData = "<< MAXDATAPOINTS << " L,R,h " << a_node->m_L << " " << a_node->m_R  << " " << a_node->m_holes << endl; 
    ChoosePartition(parVars, min_fill);

    // cout << "After choose partition" << endl;
    ...
    *a_newNode = AllocNode();
    ...
    //    a_node->m_branch.clear();
    // TODO
    // THEN WE JUST HAVE TO LOAD THE NODES DIFFERENTLY

    // cout << "at the end of splitLeaf before load leaves, MBRs: " << endl;
    // for(int i =0; i < NUMDIMS; i++) cout << (parVars->m_cover[0]).m_min[i] << " " << (parVars->m_cover[0]).m_max[i] << " ";
    // cout << endl;
    // for(int i =0; i < NUMDIMS; i++) cout << (parVars->m_cover[1]).m_min[i] << " " << (parVars->m_cover[1]).m_max[i] << " ";
    // cout << endl;


    LoadLeaves(a_node, *a_newNode, parVars);
    // cout << "After loadleaves" << endl;
    a_node->m_filled_spare_space = 0;
    (*a_newNode)->m_filled_spare_space = 0;
    a_coverRect = (parVars->m_cover[0]);
    a_newCoverRect = (parVars->m_cover[1]);

    // cout << "at the end of splitLeaf, MBRs: " << endl;
    // for(int i =0; i < NUMDIMS; i++) cout << a_coverRect.m_min[i] << " " << a_coverRect.m_max[i] << " ";
    // cout << endl;
    // for(int i =0; i < NUMDIMS; i++) cout << a_newCoverRect.m_min[i] << " " << a_newCoverRect.m_max[i] << " ";
    // cout << endl;
...
// Load branch buffer with branches from full node plus the extra branch.
RTREE_TEMPLATE
void RTREE_QUAL::GetBranches(Node *a_node,PartitionVarsLeaf *a_parVars) {
    //            ASSERT(a_node);
    //            ASSERT(a_branch);
...
// but I have to include it any way
//            a_parVars->m_branchBuf[index] = *m_data_arr[a_node->m_L + index];
            if(m_data_arr_ids[index] == 502037){
                cout << "putting item 502037 into buffer :";
                for(int j = 0; j< NUMDIMS;j++) cout << m_data_arr_mins[index][j] << " " << m_data_arr_maxes[index][j] << " ";
                cout << endl;
            }
            a_parVars->m_branchBuf.push_back(Branch(NULL, Rect(m_data_arr_mins[index], m_data_arr_maxes[index]), m_data_arr_ids[index]));
        } else {
            // a_parVars->m_branchBuf[index] = a_node->m_branch[index];
            ASSERT(0);
        }
    ...
    for(int index = 0; index < a_node->m_filled_spare_space; index++){
        a_parVars->m_branchBuf.push_back(Branch(NULL, Rect(a_node->m_spare_mins + index*NUMDIMS, a_node->m_spare_maxes + index*NUMDIMS), a_node->m_spare_ids[index]));
    }

    //    a_parVars->m_branchBuf.push_back(*a_branch);
    // a_parVars->m_branchBuf[max_children] = *a_branch;
    a_parVars->m_branchCount = (a_node->m_R - a_node->m_L - a_node->m_holes) + a_node->m_filled_spare_space;

    // Calculate rect containing all in the set
    a_parVars->m_coverSplit = a_parVars->m_branchBuf[0].m_rect;
    for (int index = 1; index < a_parVars->m_branchCount; ++index) {
        a_parVars->m_coverSplit = CombineRect(&a_parVars->m_coverSplit, &a_parVars->m_branchBuf[index].m_rect);
    }
    //    cout << "calling crv in get branches\n";
    a_parVars->m_coverSplitArea = CalcRectVolume(&a_parVars->m_coverSplit);
...
RTREE_TEMPLATE
void RTREE_QUAL::ChoosePartition(PartitionVarsLeaf *a_parVars, int a_minFill) {
    //            ASSERT(a_parVars);

    float biggestDiff;
    int group, chosen = 0, betterGroup = 0;

    // cout << "before initparvars" << endl;
    InitParVars(a_parVars, a_parVars->m_branchCount, a_minFill);
    // cout << "after parvars" << endl;
    PickSeeds(a_parVars);
    // cout << "after pickseed"<<endl;
...
void RTREE_QUAL::LoadNodes(Node *a_nodeA, Node *a_nodeB, PartitionVars *a_parVars) {
    ...
    for (int index = 0; index < a_parVars->m_total; ++index) {
        ...
        // cout << "in load nodes" << endl;
        // cout << "count[0], count[1] " << a_parVars->m_count[0] << " " << a_parVars->m_count[1] << " minfill: " << a_parVars->m_minFill << endl;
...
RTREE_TEMPLATE
void RTREE_QUAL::LoadLeaves(Node *a_nodeA, Node *a_nodeB, PartitionVarsLeaf *a_parVars) {
    // cout << "in load leaves  for node " << a_nodeA->m_id << endl;
    ASSERT(a_nodeA->m_level == 0);
    ASSERT(a_nodeB->m_level == 0);

    if(a_nodeA->m_id == m_youngest_leaf->m_id)
    {
        #ifdef stats
            count_total_holes -= m_youngest_leaf->m_holes;
        #endif
        int zeroP = a_nodeA->m_L+DEFAULT_HOLES, oneP=a_nodeA->m_L+2*DEFAULT_HOLES+a_parVars->m_count[0];
        for(int i = 0; i < a_parVars->m_total;i++) {
            if(a_parVars->m_partition[i] == 0)
            {
                for(int j = 0 ; j< NUMDIMS; j ++)
                {
                    m_data_arr_mins[zeroP][j] = a_parVars->m_branchBuf[i].m_rect.m_min[j];
                    m_data_arr_maxes[zeroP][j] = a_parVars->m_branchBuf[i].m_rect.m_max[j];
                }
                m_data_arr_ids[zeroP] = a_parVars->m_branchBuf[i].m_data;
                zeroP++;
            }
            else
            {
                for(int j = 0 ; j< NUMDIMS; j ++)
                {
                    m_data_arr_mins[oneP][j] = a_parVars->m_branchBuf[i].m_rect.m_min[j];
                    m_data_arr_maxes[oneP][j] = a_parVars->m_branchBuf[i].m_rect.m_max[j];
                }
                m_data_arr_ids[oneP] = a_parVars->m_branchBuf[i].m_data;
                oneP++;
            }
        }
        a_nodeA->m_holes = DEFAULT_HOLES;
        a_nodeA->m_R = a_nodeA->m_L+DEFAULT_HOLES+a_parVars->m_count[0];
        a_nodeB->m_L = a_nodeA->m_R;
        a_nodeB->m_holes = DEFAULT_HOLES;
        a_nodeB->m_R = a_nodeB->m_L + DEFAULT_HOLES + a_parVars->m_count[1];
        a_nodeA->m_younger_brother = a_nodeB;
        a_nodeB->m_older_brother = a_nodeA;
        a_nodeB->m_younger_brother = NULL;
        m_youngest_leaf = a_nodeB;

        a_nodeA->m_count = a_nodeA->m_R - a_nodeA->m_L - a_nodeA->m_holes;
        a_nodeB->m_count = a_nodeB->m_R - a_nodeB->m_L - a_nodeB->m_holes;

        a_nodeA->m_regular = (a_nodeA->m_count < MAXDATAPOINTS);
        a_nodeB->m_regular = (a_nodeB->m_count < MAXDATAPOINTS);
        #ifdef stats
            count_total_holes += 2*DEFAULT_HOLES;
        #endif
        return;
    }
    // if there's space near the original node, we should just fit them both there
    // if not, then we move node B to the end and all the other ones to their spots...
    // cout << "before sort partition: " << endl;
    // for(int i =0; i < a_parVars->m_branchCount; i++) cout << a_parVars->m_partition[i] << " ";
    // cout << endl;
    // for(int i =0; i < a_parVars->m_branchCount - 1; i++) {
    //     if(m_data_arr_ids[i + a_nodeA->m_L + a_nodeA->m_holes] == 152010){
    //         cout << "before 296535 is in position " << i << " in partition and " << i + a_nodeA->m_L + a_nodeA->m_holes << " with offset and the class is: "  << a_parVars->m_partition[i] << endl;
    //     }
    // }
    // cout << "before sort ids: " << endl;
    // for(int i =0; i < a_parVars->m_branchCount - 1; i++) cout << m_data_arr_ids[i + a_nodeA->m_L + a_nodeA->m_holes] << " ";
    // cout << endl;
    // cout << "before sort" << endl;
    // for(int i =0; i < (a_nodeA->m_R - a_nodeA->m_L - a_nodeA->m_holes); i++) {
    //     // if(m_data_arr_ids[i + a_nodeA->m_L + a_nodeA->m_holes] == 502037){
    //         cout << m_data_arr_ids[i + a_nodeA->m_L + a_nodeA->m_holes]  << " before is in position " << i << " in partition and " << i + a_nodeA->m_L + a_nodeA->m_holes << " with offset and the class is: "  << a_parVars->m_partition[i] << endl;
    //     // }
    // }
    // cout << "from " << a_nodeA->m_L + a_nodeA->m_holes << " to " << a_nodeA->m_R - 1 << " offset " << a_nodeA->m_L + a_nodeA->m_holes << " bcount " << a_parVars->m_branchCount << " filled space " << a_nodeA->m_filled_spare_space << endl;
    int split_position = partition_on_one(&(a_parVars->m_partition), a_nodeA->m_L + a_nodeA->m_holes, a_nodeA->m_R - 1, a_nodeA->m_L + a_nodeA->m_holes);
    // cout << "after partition: " << split_position << " branch_count " << a_parVars->m_branchCount << " offset:  " << (a_nodeA->m_L + a_nodeA->m_holes) << endl;
    // for(int i =0; i < a_parVars->m_branchCount; i++) cout << a_parVars->m_partition[i] << " ";
    // cout << endl;
    // cout << "after sort ids: " << endl;
    // for(int i =0; i < a_parVars->m_branchCount - 1; i++) cout << m_data_arr_ids[i + a_nodeA->m_L + a_nodeA->m_holes] << " ";
    // cout << endl;
    // cout << "partition around sp: " <<  a_parVars->m_partition[split_position - 1 - (a_nodeA->m_L + a_nodeA->m_holes)] << " " << a_parVars->m_partition[split_position - (a_nodeA->m_L + a_nodeA->m_holes)] << " " << a_parVars->m_partition[split_position + 1 - (a_nodeA->m_L + a_nodeA->m_holes)] << endl;
    // cout << "after sort" << endl;
    // for(int i =0; i < (a_nodeA->m_R - a_nodeA->m_L - a_nodeA->m_holes); i++) {
    //     // if(m_data_arr_ids[i + a_nodeA->m_L + a_nodeA->m_holes] == 502037){
    //         cout << m_data_arr_ids[i + a_nodeA->m_L + a_nodeA->m_holes]  << " after is in position " << i << " in partition and " << i + a_nodeA->m_L + a_nodeA->m_holes << " with offset and the class is: "  << a_parVars->m_partition[i] << endl;
    //     // }
    // }

    // patwork solution

    // cout << "##### " << split_position << endl;
    if(a_parVars->m_partition[split_position - (a_nodeA->m_L + a_nodeA->m_holes)] == 0 && split_position != a_nodeA->m_R) split_position++;

    // cout << "SPlitposition =  " << split_position << endl;
    // for(int ii = a_nodeA->m_L + a_nodeA->m_holes; ii < a_nodeA->m_R; ii++)
    // {  
    //     if(m_data_arr_ids[ii] == 502037) cout << "Object 502037 is in position " << ii << endl;

    // }
    

    
    if(a_nodeA->m_holes >= (a_nodeA->m_filled_spare_space)){
        #ifdef stats
            count_total_holes -= (a_nodeA->m_filled_spare_space);
        #endif
        // cout << "In case 1" << endl;

        int start_of_spares = a_nodeA->m_R - a_nodeA->m_L - a_nodeA->m_holes;
        a_nodeB->m_R = a_nodeA->m_R;
        a_nodeA->m_R = split_position;
        a_nodeB->m_L = split_position;
        a_nodeB->m_holes = 0;



        for(int index = start_of_spares; index < a_parVars->m_total; index++){
            if(a_parVars->m_partition[index] == 0){
                // cout << "partition is 0 " << endl;

                for(int i = 0; i < NUMDIMS; i++){
                    m_data_arr_mins[a_nodeA->m_L + a_nodeA->m_holes - 1][i] = a_parVars->m_branchBuf[index].m_rect.m_min[i];
                    m_data_arr_maxes[a_nodeA->m_L + a_nodeA->m_holes - 1][i] = a_parVars->m_branchBuf[index].m_rect.m_max[i];
                }
                m_data_arr_ids[a_nodeA->m_L + a_nodeA->m_holes - 1] = a_parVars->m_branchBuf[index].m_data;
                ...
                // cout << "Partittion is 1" << endl;
                for(int i = 0; i < NUMDIMS; i++){
                    m_data_arr_mins[a_nodeA->m_L + a_nodeA->m_holes - 1][i] = m_data_arr_mins[a_nodeA->m_R - 1][i];
                    m_data_arr_maxes[a_nodeA->m_L + a_nodeA->m_holes - 1][i] = m_data_arr_maxes[a_nodeA->m_R - 1][i];
                }
                m_data_arr_ids[a_nodeA->m_L + a_nodeA->m_holes - 1] = m_data_arr_ids[a_nodeA->m_R - 1];

                // cout << " copied from " << split_position - 1 << " to " << a_nodeA->m_L + a_nodeA->m_holes - 1 << endl;

                for(int i = 0; i < NUMDIMS; i++){
                    m_data_arr_mins[a_nodeA->m_R - 1][i] = a_parVars->m_branchBuf[index].m_rect.m_min[i];
                    m_data_arr_maxes[a_nodeA->m_R - 1][i] = a_parVars->m_branchBuf[index].m_rect.m_max[i];
                }
                m_data_arr_ids[a_nodeA->m_R - 1] = a_parVars->m_branchBuf[index].m_data;

                // cout << "copied from a_branch to " << split_position - 1 << endl;
        ...
        // cout << "case 1 there were holes l. h. r. sp "  << a_nodeA->m_L << " " << a_nodeA->m_holes << " " << a_nodeA->m_R << " " << split_position << endl;
        // then let's write the last item in the buffer(the nwe item) into the first spot available(L + h - 1)
        // and then sort that space([L + h -1 : R]) based on partition
        // then the ones for nodeA(having partition 0) will be at the start all next to each other
        // and the other ones will be away
        // then we can shift some to accommodate the splitting of the holes
        // TODO: I know this shifting thing will be just as annoying and buggy as it was when we did it for cracking, so I'll leave it for when Dinos is here:)
        // for(int i = 0; i < NUMDIMS; i++){
        //     m_data_arr_mins[a_nodeA->m_L + a_nodeA->m_holes - 1][i] = a_parVars->m_branchBuf[a_parVars->m_branchCount - 1].m_rect.m_min[i];
        //     m_data_arr_maxes[a_nodeA->m_L + a_nodeA->m_holes - 1][i] = a_parVars->m_branchBuf[a_parVars->m_branchCount - 1].m_rect.m_max[i];
        // }
        // m_data_arr_ids[a_nodeA->m_L + a_nodeA->m_holes - 1] = a_parVars->m_branchBuf[a_parVars->m_branchCount - 1].m_data;

        // WRONG!!! THE MAPS ARE SHIFTED IN THE WORNG WAY.
        // NEW VERSION ABOVE

        //wait a sec 
        // so I chnaged the QS to swap the data arrs instead of the tba mins
        // and also just completely removed all trace of the tba mins
        // I think the below makes sense, but I have to leave so tomorrow we'll actually figure it out:)))
        // IDEA: maybe it can return where it is different
        // IDEA: we couldd just do one partition on the map values for pivot=1 I think but again maybe im wrong.... --> Dinos agrees he is sick tho...
        // quickSort(a_parVars->m_partition, a_nodeA->m_L + a_nodeA->m_holes - 1, a_nodeA->m_R, a_nodeA->m_L + a_nodeA->m_holes - 1);
        // int split_position = partition_on_one(a_parVars->m_partition, a_nodeA->m_L + a_nodeA->m_holes - 1, a_nodeA->m_R, a_nodeA->m_L + a_nodeA->m_holes - 1);
        // TODO: split the holes later, for now NodeA gets all of them
        ...
        // ASSERT(a_nodeA->m_regular);
        // ASSERT(a_nodeB->m_regular);

        
        // setting up leaf linked list
        a_nodeB->m_younger_brother = a_nodeA->m_younger_brother;
        if(a_nodeB->m_younger_brother != NULL){
            (a_nodeB->m_younger_brother)->m_older_brother = a_nodeB;
        }
        else{
            // A was the younegst leaf
            m_youngest_leaf = a_nodeB;
        }
        a_nodeB->m_older_brother = a_nodeA;
        a_nodeA->m_younger_brother = a_nodeB;
        ...
        int start_of_spares = a_nodeA->m_R - a_nodeA->m_L - a_nodeA->m_holes;
        // cout << "case 2 no holes  l. h. r. sp "  << a_nodeA->m_L << " " << a_nodeA->m_holes << " " << a_nodeA->m_R << " " << split_position << endl;
        // I will think about this later...

        // move all the 0 stuff to the end
        // first we will check to see if there is space for the rest of 1 to fit in the place we are emptying
        if((a_parVars->m_count[1] - a_nodeA->m_R + split_position) > (split_position - a_nodeA->m_L)){
            // there is not enoughs space for later

            // cout << a_nodeA->m_R  << " " <<  a_nodeA->m_L << " " << a_nodeA->m_holes << " " << split_position << " " << a_parVars->m_count[0] << " " << a_parVars->m_count[1] << " " << a_nodeA->m_filled_spare_space << endl;
             
            if(a_nodeA->m_R - a_nodeA->m_L >= a_parVars->m_count[0]){

                // cout << "there is space for the 0 to stay" << endl;

                int last_filled_spot_in_arr = m_youngest_leaf->m_R;

                // cout << "memcpy from " << split_position << " thi smany " << (a_nodeA->m_R - split_position) << " to " << last_filled_spot_in_arr + DEFAULT_HOLES << endl;

                memcpy(m_data_arr_mins[last_filled_spot_in_arr + DEFAULT_HOLES], m_data_arr_mins[split_position], NUMDIMS*(a_nodeA->m_R - split_position)*sizeof(float));
                memcpy(m_data_arr_maxes[last_filled_spot_in_arr + DEFAULT_HOLES], m_data_arr_maxes[split_position], NUMDIMS*(a_nodeA->m_R - split_position)*sizeof(float));
                memcpy(m_data_arr_ids + last_filled_spot_in_arr + DEFAULT_HOLES, m_data_arr_ids + split_position, (a_nodeA->m_R - split_position)*sizeof(int));

                // cout << "after memcpy" << endl;

                // B = 1, A = 0
                int folan = (a_nodeA->m_R - split_position);
                a_nodeB->m_L = last_filled_spot_in_arr;
                last_filled_spot_in_arr += folan + DEFAULT_HOLES;
                a_nodeB->m_R = last_filled_spot_in_arr;
                a_nodeB->m_holes = DEFAULT_HOLES;
                #ifdef stats
                    count_total_holes += DEFAULT_HOLES;
                #endif

                // cout << "after some fixing" << endl;


                // set the holes for the right bro
                // set the R for A
                // if(a_nodeA->m_younger_brother == NULL) cout << "anodeA is youngest!" << endl;
                (a_nodeA->m_younger_brother)->m_L = split_position;
                (a_nodeA->m_younger_brother)->m_holes += folan;
                #ifdef stats
                    count_total_holes += folan;
                #endif
                a_nodeA->m_R = split_position;
                ...
                for(int index = start_of_spares; index < a_parVars->m_total; index++){
                    ...
                    if(a_parVars->m_partition[index] == 0){
                        ...
                        if((a_nodeA->m_younger_brother)->m_holes > 0){
                            // put them after it
                            // cout << "case 2 1 a branch to 0" << endl;
                            // then it goes to the end with the rest of 0
                            memcpy(m_data_arr_mins[a_nodeA->m_R], a_parVars->m_branchBuf[index].m_rect.m_min, NUMDIMS*sizeof(float));
                            // memcpy(m_data_arr_maxes[last_filled_spot_in_arr], tba_maxes[this_tba_start], k*NUMDIMS*sizeof(float));
                            memcpy(m_data_arr_maxes[a_nodeA->m_R], a_parVars->m_branchBuf[index].m_rect.m_max, NUMDIMS*sizeof(float));
                            // memcpy(m_data_arr_ids + last_filled_spot_in_arr, tba_ids + this_tba_start, k*sizeof(int));
                            m_data_arr_ids[a_nodeA->m_R] = a_parVars->m_branchBuf[index].m_data;

                            a_nodeA->m_R++;
                            (a_nodeA->m_younger_brother)->m_holes--;
                            #ifdef stats
                            count_total_holes--;
                            #endif
                            (a_nodeA->m_younger_brother)->m_L++;

                        }
                        else{
                            // start filling the holes of A
                            // cout << "case 2 1 a branch to 1" << endl;
                            memcpy(m_data_arr_mins[a_nodeA->m_L + a_nodeA->m_holes - 1], a_parVars->m_branchBuf[index].m_rect.m_min, NUMDIMS*sizeof(float));
                            // memcpy(m_data_arr_maxes[last_filled_spot_in_arr], tba_maxes[this_tba_start], k*NUMDIMS*sizeof(float));
                            memcpy(m_data_arr_maxes[a_nodeA->m_L + a_nodeA->m_holes - 1], a_parVars->m_branchBuf[index].m_rect.m_max, NUMDIMS*sizeof(float));
                            // memcpy(m_data_arr_ids + last_filled_spot_in_arr, tba_ids + this_tba_start, k*sizeof(int));
                            m_data_arr_ids[a_nodeA->m_L + a_nodeA->m_holes - 1] = a_parVars->m_branchBuf[index].m_data;

                            // cout << "copied from a branch to " << split_position - 1 << endl;
                        ...
                        // cout << "-- case 2 1 a branch to 0" << endl;
                        // then it goes to the end with the rest of 0
                        memcpy(m_data_arr_mins[a_nodeB->m_R], a_parVars->m_branchBuf[index].m_rect.m_min, NUMDIMS*sizeof(float));
                        // memcpy(m_data_arr_maxes[last_filled_spot_in_arr], tba_maxes[this_tba_start], k*NUMDIMS*sizeof(float));
                        memcpy(m_data_arr_maxes[a_nodeB->m_R], a_parVars->m_branchBuf[index].m_rect.m_max, NUMDIMS*sizeof(float));
                        // memcpy(m_data_arr_ids + last_filled_spot_in_arr, tba_ids + this_tba_start, k*sizeof(int));
                        m_data_arr_ids[a_nodeB->m_R] = a_parVars->m_branchBuf[index].m_data;
                ...
                // a_nodeB->m_older_brother = a_nodeA->m_older_brother;
                // if(a_nodeB->m_older_brother != NULL){
                //     (a_nodeB->m_older_brother)->m_younger_brother = a_nodeB;
                // }
                // else{
                //     m_oldest_leaf = a_nodeB;
                // }

                m_youngest_leaf->m_younger_brother = a_nodeB;
                a_nodeB->m_older_brother = m_youngest_leaf;
                a_nodeB->m_younger_brother = NULL;
                m_youngest_leaf = a_nodeB;
            }
            else {
                // neither fit. Move both to the end.
                // cout << "Both dont fit l.id " << a_nodeA->m_id << " ybro.id " << a_nodeA->m_younger_brother->m_id << " obro.id " << a_nodeA->m_older_brother->m_id << endl;

                (a_nodeA->m_younger_brother)->m_holes += (a_nodeA->m_R - a_nodeA->m_L);
                (a_nodeA->m_younger_brother)->m_L -= (a_nodeA->m_R - a_nodeA->m_L);
                #ifdef stats
                    count_total_holes += (a_nodeA->m_R - a_nodeA->m_L - a_nodeA->m_holes);
                #endif
                ...
                for(int index = start_of_spares; index < a_parVars->m_total; index++){
                    if(a_parVars->m_partition[index] == 1){
                        memcpy(m_data_arr_mins[a_nodeB->m_R], a_parVars->m_branchBuf[index].m_rect.m_min, NUMDIMS*sizeof(float));
                        memcpy(m_data_arr_maxes[a_nodeB->m_R], a_parVars->m_branchBuf[index].m_rect.m_max, NUMDIMS*sizeof(float));
                        m_data_arr_ids[a_nodeB->m_R] = a_parVars->m_branchBuf[index].m_data;
                        a_nodeB->m_R++;
                ...
                last_filled_spot_in_arr = a_nodeB->m_R;
                memcpy(m_data_arr_mins[last_filled_spot_in_arr + DEFAULT_HOLES], m_data_arr_mins[a_nodeA->m_L + a_nodeA->m_holes], NUMDIMS*(split_position - a_nodeA->m_L - a_nodeA->m_holes)*sizeof(float));
                memcpy(m_data_arr_maxes[last_filled_spot_in_arr + DEFAULT_HOLES], m_data_arr_maxes[a_nodeA->m_L + a_nodeA->m_holes], NUMDIMS*(split_position - a_nodeA->m_L - a_nodeA->m_holes)*sizeof(float));
                memcpy(m_data_arr_ids + last_filled_spot_in_arr + DEFAULT_HOLES, m_data_arr_ids + a_nodeA->m_L + a_nodeA->m_holes, (split_position - a_nodeA->m_L - a_nodeA->m_holes)*sizeof(int));

                folan = (split_position - a_nodeA->m_L - a_nodeA->m_holes);
                a_nodeA->m_L = last_filled_spot_in_arr;
                last_filled_spot_in_arr += folan + DEFAULT_HOLES;
                a_nodeA->m_R = last_filled_spot_in_arr;
                a_nodeA->m_holes = DEFAULT_HOLES;
                #ifdef stats
                count_total_holes += 2*DEFAULT_HOLES;
                #endif
                ...
                (a_nodeA->m_younger_brother)->m_older_brother = a_nodeA->m_older_brother;
                (a_nodeA->m_older_brother)->m_younger_brother = a_nodeA->m_younger_brother;

                m_youngest_leaf->m_younger_brother = a_nodeB;
                a_nodeB->m_older_brother = m_youngest_leaf;

                a_nodeB->m_younger_brother = a_nodeA;
                a_nodeA->m_older_brother = a_nodeB;

                a_nodeA->m_younger_brother = NULL;
                m_youngest_leaf = a_nodeA;
            ...
            a_nodeB->m_R = a_nodeA->m_R;
            a_nodeB->m_L = a_nodeA->m_L;

            #ifdef stats
                count_total_holes -= a_nodeA->m_holes;
            #endif
            a_nodeB->m_holes = split_position - a_nodeA->m_L;
            #ifdef stats
                count_total_holes += split_position - a_nodeA->m_L;
            #endif
            
            int folan = (split_position - a_nodeA->m_L - a_nodeA->m_holes);
            a_nodeA->m_L = last_filled_spot_in_arr;
            last_filled_spot_in_arr += folan + DEFAULT_HOLES;
            ...
            for(int index = start_of_spares; index < a_parVars->m_total; index++){
                ...
                if(a_parVars->m_partition[index] == 0){
                    ...
                    // cout << " +++ case 2 1 a branch to 1" << endl;
                    memcpy(m_data_arr_mins[a_nodeB->m_L + a_nodeB->m_holes - 1], a_parVars->m_branchBuf[index].m_rect.m_min, NUMDIMS*sizeof(float));
                    // memcpy(m_data_arr_maxes[last_filled_spot_in_arr], tba_maxes[this_tba_start], k*NUMDIMS*sizeof(float));
                    memcpy(m_data_arr_maxes[a_nodeB->m_L + a_nodeB->m_holes - 1], a_parVars->m_branchBuf[index].m_rect.m_max, NUMDIMS*sizeof(float));
                    // memcpy(m_data_arr_ids + last_filled_spot_in_arr, tba_ids + this_tba_start, k*sizeof(int));
                    m_data_arr_ids[a_nodeB->m_L + a_nodeB->m_holes - 1] = a_parVars->m_branchBuf[index].m_data;
            ...
            // cout << "case 1 1 A after : " << a_nodeA->m_L << " " << a_nodeA->m_holes << " " << a_nodeA->m_R << endl;
            // cout << "case 1 1 B after: " << a_nodeB->m_L << " " << a_nodeB->m_holes << " " << a_nodeB->m_R << endl;
            ...
            m_youngest_leaf->m_younger_brother = a_nodeA;
            a_nodeA->m_older_brother = m_youngest_leaf;
            a_nodeA->m_younger_brother = NULL;
            m_youngest_leaf = a_nodeA;
...
// Initialize a PartitionVars structure.
RTREE_TEMPLATE
void RTREE_QUAL::InitParVars(PartitionVarsLeaf *a_parVars, int a_maxRects, int a_minFill) {
    //            ASSERT(a_parVars);

    // cout << "in initparvars " << endl;
    a_parVars->m_count[0] = a_parVars->m_count[1] = 0;
    a_parVars->m_area[0] = a_parVars->m_area[1] = (float) 0;
    a_parVars->m_total = a_maxRects;
    a_parVars->m_minFill = a_minFill;
...
RTREE_TEMPLATE
void RTREE_QUAL::PickSeeds(PartitionVarsLeaf *a_parVars) {
    int seed0 = 0, seed1 = 0;
    float worst, waste;
    float area[a_parVars->m_total + 1];
...
// Put a branch in one of the groups.
RTREE_TEMPLATE
void RTREE_QUAL::Classify(int a_index, int a_group, PartitionVarsLeaf *a_parVars) {
    //    cout << "CHECK!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! " << a_index << " " << a_parVars->m_partition[a_index] << endl;
    //            ASSERT(a_parVars);
    //            ASSERT(PartitionVars::NOT_TAKEN == a_parVars->m_partition[a_index]);
    if(a_parVars->m_branchBuf[a_index].m_data == 502037){
        cout << "classifying  502037 to " << a_group << " cover: ";
        for(int j = 0; j< NUMDIMS;j++) cout << a_parVars->m_branchBuf[a_index].m_rect.m_min[j] << " " << a_parVars->m_branchBuf[a_index].m_rect.m_max[j] << " ";
        cout << endl;
    }
    a_parVars->m_partition[a_index] = a_group;
    ...
    if(a_parVars->m_branchBuf[a_index].m_data == 502037){
        ...
        for(int j = 0; j< NUMDIMS;j++) cout << a_parVars->m_branchBuf[a_index].m_rect.m_min[j] << " " << a_parVars->m_branchBuf[a_index].m_rect.m_max[j] << " ";
        cout << endl;
        cout << "after combination cover of group is: " ;
        for(int j = 0; j< NUMDIMS;j++) cout << a_parVars->m_cover[a_group].m_min[j] << " " << a_parVars->m_cover[a_group].m_max[j] << " ";
        cout << endl;
    }
...
bool RTREE_QUAL::Cover(Rect *a_rectA, const float a_min[NUMDIMS], const float a_max[NUMDIMS]) const {
    ...
    // int count_ok_dims = 0;
    // for(int i = 0; i < NUMDIMS; i++){
    //     if((a_min[i] > a_rectA->m_min[i] || fabs(a_min[i] - a_rectA->m_min[i]) < EPSILON) && (a_min[i] < a_rectA->m_max[i] || fabs(a_min[i] - a_rectA->m_max[i]) < EPSILON)  &&
    //         (a_max[i] > a_rectA->m_min[i] || fabs(a_max[i] - a_rectA->m_min[i]) < EPSILON) && (a_max[i] < a_rectA->m_max[i] || fabs(a_max[i] - a_rectA->m_max[i]) < EPSILON ) ){
    //             count_ok_dims++;
    //         }
    // }
    // return count_ok_dims == NUMDIMS;


    // int count_ok_dims = 0;
    // for(int i = 0; i < NUMDIMS; i++){
    //     if((a_min[i] >= a_rectA->m_min[i]) && (a_min[i] <= a_rectA->m_max[i])  &&
    //         (a_max[i] >= a_rectA->m_min[i]) && (a_max[i] <= a_rectA->m_max[i]) ){
    //             count_ok_dims++;
    //         }
    // } 
    // return count_ok_dims == NUMDIMS;
    ...
    // int count_ok_dims = 0;
    // for(int i = 0; i < NUMDIMS; i++){
    //     if((a_min[i] >= (a_rectA->m_min[i] - EPSILON)) && (a_min[i] <= (a_rectA->m_max[i] + EPSILON) )  &&
    //         (a_max[i] >= (a_rectA->m_min[i] - EPSILON)) && (a_max[i] <= (a_rectA->m_max[i] + EPSILON)) ){
    //             count_ok_dims++;
    //         }
    // }
    // return count_ok_dims == NUMDIMS;
...
//                }
            ...
            m_root->m_parent = NULL;
                ...
                count_irregular_leaves--;
                // auto t = CountLeaves();
                // cout << "in add_ltas removed one leaf " << count_leaves << " " << t.first + t.second << endl;
                count_internal_nodes++;
            #endif
...
void RTREE_QUAL::ripple_lite(const Branch &a_branch, Node *a_node){
    ...
    if(a_node->m_id == m_youngest_leaf->m_id){
        // append to the end of this leaf
        // memcpy(m_data_arr_ids + this_leaf->m_R, tba_ids + this_tba_start, k*sizeof(int));
    ...
    // otherwise
    // then just move this leaf to the end with its tba
    (a_node->m_younger_brother)->m_L = a_node->m_L;
    (a_node->m_younger_brother)->m_holes += (a_node->m_R - a_node->m_L);
    ...
    if(a_node->m_id == m_oldest_leaf->m_id){
        (a_node->m_younger_brother)->m_older_brother = NULL;
        m_oldest_leaf = a_node->m_younger_brother;
        ASSERT((a_node->m_younger_brother)->m_L == 0);

    }
    else{
        (a_node->m_older_brother)->m_younger_brother = a_node->m_younger_brother;
        (a_node->m_younger_brother)->m_older_brother = a_node->m_older_brother;
    }

    a_node->m_older_brother = m_youngest_leaf;
    m_youngest_leaf->m_younger_brother = a_node;
    a_node->m_younger_brother = NULL;
    a_node->m_holes = DEFAULT_HOLES;
    a_node->m_count = a_node->m_R - a_node->m_L - a_node->m_holes;
    m_youngest_leaf = a_node;
    return;
...
void RTREE_QUAL::ripple_lite_v2(float* this_tba_mins, float* this_tba_maxes, int* this_tba_ids, int this_tba_count, Node* this_leaf, vector<pair<Node*, Branch>> *leaves_to_add_later){
    ...
    // if(this_leaf->m_younger_brother == NULL) cout << "shiiiiiit " << endl;
    // otherwise
    // then just move this leaf to the end with its tba
    ...
    if(this_leaf->m_R - this_leaf->m_L - this_leaf->m_holes > MOVING_THRESHOLD && (k < (this_leaf->m_R - this_leaf->m_L))){
        ...
        // TODO: remember to fix its rect too
        // TODO: fix this_leaf's MBR, and also fix newBranch's MBR
        ...
        if(stochastic_crack_index != this_L + this_holes && stochastic_crack_index != this_R){
            ...
            if(this_R - stochastic_crack_index < (stochastic_crack_index - this_L - this_holes)){
                ...
                if(this_holes + this_R - stochastic_crack_index >= k){
                    ...
                    (this_leaf->m_younger_brother)->m_holes += (this_R - stochastic_crack_index);
                    #ifdef stats
                    count_total_holes += (this_R - stochastic_crack_index);
                    #endif
                    (this_leaf->m_younger_brother)->m_L -= (this_R - stochastic_crack_index);
                    ...
                    for(int i = 0; i < this_tba_count; i++){
                        ...
                        if((this_tba_maxes + i * NUMDIMS)[stochastic_crack_axis] <= stochastic_crack_pivot){
                            ...
                            if((this_leaf->m_younger_brother)->m_holes > 0){
                                ...
                                this_leaf->m_R ++;
                                (this_leaf->m_younger_brother)->m_holes--;
                                #ifdef stats
                                count_total_holes--;
                                #endif
                                (this_leaf->m_younger_brother)->m_L++;
                    ...
                    // fix the linked list
                    newNode->m_older_brother = m_youngest_leaf;
                    m_youngest_leaf->m_younger_brother = newNode;
                    newNode->m_younger_brother = NULL;
                    m_youngest_leaf = newNode;
                    ...
                    if(stochastic_crack_index - this_L >= k){
                        ...
                        // neither fit, we have to move both. TODO!!!!!!!!!!!!!!!!!!!!

                        (this_leaf->m_younger_brother)->m_holes += (this_R - this_L);
                        (this_leaf->m_younger_brother)->m_L -= (this_R - this_L);
                        #ifdef stats
                        ...
                        // fix its linked list
                        (this_leaf->m_younger_brother)->m_older_brother = this_leaf->m_older_brother;
                        if(this_leaf->m_older_brother != NULL){
                            // not head
                            (this_leaf->m_older_brother)->m_younger_brother = this_leaf->m_younger_brother;
                        }
                        else{
                        ...
                        last_filled_spot_in_arr = m_youngest_leaf->m_R;
                ...
                if(stochastic_crack_index - this_L >= k){
                    ...
                    if(this_holes + this_R - stochastic_crack_index >= k){
                        ...
                        int last_filled_spot_in_arr = m_youngest_leaf->m_R;
                        Rect trash_rect;
                        // cout << "check in 2 " << last_filled_spot_in_arr << endl;
            ...
            // cout << "before mem1 from " <<  this_leaf->m_L + this_leaf->m_holes << " to " << last_filled_spot_in_arr << " how many " <<  this_leaf->m_R - this_leaf->m_L - this_leaf->m_holes << endl;
            ...
            // cout << "After mem1" << endl;

            folan = (this_leaf->m_R - this_leaf->m_L - this_leaf->m_holes);
            this_leaf->m_L = last_filled_spot_in_arr;
            last_filled_spot_in_arr += folan + DEFAULT_HOLES;
            // cout << "After mem11" << endl;
            ...
            this_leaf->m_count = this_leaf->m_R - this_leaf->m_L - this_leaf->m_holes;
            m_youngest_leaf = this_leaf;
            // cout <<"end of ripple" << endl;    
        }
        
        
    }
    else{
        // cout << "small enough to mov" <<endl;
...
bool RTREE_QUAL::printLeafBoundsOfDataRec(Node *a_node, int data_index){
    if(a_node->IsInternalNode()){
        for(int i = 0; i < a_node->m_count; i++){
            ...
            if(print_or_not){
                cout << "id: " << a_node->m_id << " level: " << a_node->m_level << " : " ;
        ...
        for(int j = a_node->m_L + a_node->m_holes; j < a_node->m_R; j++){
            if(m_data_arr_ids[j] == data_index){
                cout << "in leaf " << a_node->m_id << " data was: " << endl;
...
int RTREE_QUAL::getCountOfHoles(){
    int hole_count = 0;
    Node* this_node = m_oldest_leaf;

    while(this_node != NULL){
        hole_count += this_node->m_holes;
        this_node = this_node->m_younger_brother;
    }
    return hole_count;
}
...
bool RTREE_QUAL::deleteRec(Node* a_node, float a_min[NUMDIMS],  float a_max[NUMDIMS],  DATATYPE &a_dataId){
    ...
    for(int i = 0; i < a_node->m_filled_spare_space; i++){
        if(a_node->m_spare_ids[i] == a_dataId){
            ...
            // level 1 tight: re calc MBR if it is internal
            #if defined(tighter) || defined(tighter2)
                if(a_node->m_level > 0){
                    Rect trash_rect;
                    Rect new_mbr = a_node->m_branch[0].m_rect;
                    // the children
                    for(int k = 1; k < a_node->m_count; k++){
                        new_mbr = CombineRect(&new_mbr, &(a_node->m_branch[k].m_rect));
                    }
                    // the spares
                    for(int k =0; k < a_node->m_filled_spare_space; k++){
                        trash_rect = Rect(a_node->m_spare_mins + k*NUMDIMS, a_node->m_spare_maxes + k*NUMDIMS);
                        new_mbr = CombineRect(&new_mbr, &trash_rect);
                    }
                    if(a_node->m_parent !=  NULL){
                        // not root
                        ((a_node->m_parent)->m_branch[getNodeBranchIndex(a_node)]).m_rect = new_mbr;
                    }
    ...
    if(a_node->m_level > 0){
        // if(a_node->m_id == 72877 || a_node->m_id == 155386 || a_node->m_id == 131221 || a_node->m_id == 39 || a_node->m_id == 193717){
        //     cout << "in delete, visitng node " << a_node->m_id << endl;
        // }
        // if(a_node->m_id == 193717){
        //     Node* trash;
        //     for(int folan = 0; folan < a_node->m_count; folan++){
        //         if(a_node->m_branch[folan].m_child->m_id == 193721){
        //             cout << "leaf bounds: ";
        //             for(int filan = 0; filan < NUMDIMS; filan++) cout << a_node->m_branch[folan].m_rect.m_min[filan] << " " << a_node->m_branch[folan].m_rect.m_max[filan] << " ";
        //             cout << endl;
        //             cout << "tbd bounds: " << a_min[0] << " " << a_max[0] << " " << a_min[1] << " " << a_max[1] << endl;
        //             cout << "result of cover function: " << Cover(&(a_node->m_branch[folan].m_rect), a_min, a_max) << endl;
        //             // a_min[index] < a_rectA->m_min[index]
        //             cout << "condition : " << (fabs(a_max[0] - a_node->m_branch[folan].m_rect.m_max[0]) < EPSILON) << " " << a_max[0] - a_node->m_branch[folan].m_rect.m_max[0] << endl;
        //             // cout << "condition: " << (a_max[0] <= a_node->m_branch[folan].m_rect.m_max[0])  << " "  <<  (a_max[0] <= a_node->m_branch[folan].m_rect.m_max[0]) << endl;
        //             trash = a_node->m_branch[folan].m_child;
        //             for(int filan = trash->m_L + trash->m_holes; filan < trash->m_R; filan++){
        //                 if(m_data_arr_ids[filan] == 4338016) cout << "4338016 data is here " << endl;
        //             }
        ...
        for(int i = 0; i < a_node->m_count; i++){
            ...
            if(Cover(&(a_node->m_branch[i].m_rect), a_min, a_max)){
                if(deleteRec(a_node->m_branch[i].m_child, a_min, a_max, a_dataId)){
                    // level 2 tight: recalc MBRS if the item was found in this path
                    #ifdef tighter2
                        Rect trash_rect;
                        Rect new_mbr = a_node->m_branch[0].m_rect;
                        // the children
        ...
        // if(a_node->m_id == 193721){
        //     cout << "in delete visiting leaf " << a_node->m_id << endl;
        // }
        // is leaf
        // if(a_node->m_id == 898) cout << "visited leaf 898" << endl;
        #ifdef delete_timebreak
            start_time = clock();
        #endif
        for(int i = a_node->m_L + a_node->m_holes; i < a_node->m_R; i++){
            if(m_data_arr_ids[i] == a_dataId){
                ...
                a_node->m_holes++;
                // level 3 tight: recalc MBR of regular leaves
