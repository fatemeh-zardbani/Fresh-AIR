...
// copied from GQ3 stochastic version
// adding minimum threshold
// and no shuffle just move to the end
// updates mbr after each pick branch
...
void RTREE_QUAL::TraverseDownTreeTilLeaf(Node *a_node, float* tba_mins, float* tba_maxes, int* tba_ids, int tba_count, vector<overlapping_leaf_tbas> *overlapping_leaves){
    ...
    if(tba_count + a_node->m_filled_spare_space < SPARE_SPACE_LIMIT){
        // TODO
        // scan the old spares for results
        // for(int i = 0; i < a_node->m_filled_spare_space; i++) {
        //     this_rect = (Rect(a_node->m_spare_mins+i*NUMDIMS, a_node->m_spare_maxes+i*NUMDIMS));
        ...
        // update the MBR of a_node to include these
        // for(int i = 0; i < tba_count; i++) {
        ...
        // empty tba count
        // tba_count = 0;
        // they're calloced, so we just don't change them

    }
    else{
        // TODO
        // diffuse the tba and old spares tp the children -> DONE
        // to do this with the same quicksort tricks and in place assignments, we will cpy the tba to the end of spare -> DONE
        // ASSERT(tba_count + a_node->m_filled_spare_space < SPARE_SPACE_LIMIT);
        ...
        // while doing the pickbranch pass over the old spares, also update the foundCount DONE
        // also add the tba to the MBR of a_node DONE
...
bool RTREE_QUAL::Search_2027(Node *a_node, Rect *a_rect, int &a_foundCount, LeavesToAdd_v2 *all_lta, float* tba_mins, float* tba_maxes, int* tba_ids, int tba_count, vector<overlapping_leaf_tbas> *overlapping_leaves) {
    ...
    if (a_node->IsInternalNode()) {
        // if(a_node->m_id == 6674) cout << "looking at node 6674" << endl;
        ...
        // very very bad code
        if(a_node->m_count > 0){
            // if(a_node->m_id == 6674) cout << "2 looking at node 6674 " << a_node->m_filled_spare_space << endl;
            // old
            // vector<Branch> tba_to_children[a_node->m_count];
            // int a_node_branch_index;
            // if(a_node->m_parent != NULL) a_node_branch_index = getNodeBranchIndex(a_node);
            ...
            Rect this_rect;
            if(tba_count + a_node->m_filled_spare_space < SPARE_SPACE_LIMIT){
                // if(a_node->m_id == 6674) cout << "3 looking at node 6674 " << a_node->m_filled_spare_space << " " << tba_count  << endl;
                // TODO
                // scan the old spares for results
                // cout << "++++++++++++++++++++++" << endl;
                for(int i = 0; i < a_node->m_filled_spare_space; i++) {
                ...
                // if(a_node->m_id == 6674) {
                //     cout << "node mbr before: "<< endl;
                //     for(int j = 0; j < NUMDIMS; j++) cout << ((a_node->m_parent)->m_branch[a_node_branch_index].m_rect).m_min[j] << " " << ((a_node->m_parent)->m_branch[a_node_branch_index].m_rect).m_max[j] << ", ";
                //     cout << endl;
                // }
                ...
                // for(int i = 0; i < tba_count; i++) {
                //     this_rect = (Rect(tba_mins+i*NUMDIMS, tba_maxes+i*NUMDIMS));
                //     // if(a_node->m_id == 6674) cout << "combining item " << tba_ids[i] << endl;
                //     if(a_node->m_parent != NULL) {
                //         (a_node->m_parent)->m_branch[a_node_branch_index].m_rect = CombineRect(&this_rect, &((a_node->m_parent)->m_branch[a_node_branch_index].m_rect));
                //     }
                // }
                ...
                // if(a_node->m_id == 6674) cout << "4 looking at node 6674 " << a_node->m_filled_spare_space << endl;
                // TODO
                // diffuse the tba and old spares tp the children -> DONE
                // to do this with the same quicksort tricks and in place assignments, we will cpy the tba to the end of spare -> DONE
                // ASSERT(tba_count + a_node->m_filled_spare_space < SPARE_SPACE_LIMIT);
            ...
            for (int index = 0; index < a_node->m_count; ++index) {
                ...
                // if((a_node->m_branch[index].m_child)->m_id == 541094) {
                //     cout << "in parent of 541094, checking overlap..." << endl;
                ...
                if (Overlap(a_rect, &a_node->m_branch[index].m_rect)) {
                    // if((a_node->m_branch[index].m_child)->m_id == 541094) cout << "it overlapped, rec call" << endl;
        ...
        // cout << "©" << endl;
        // if(a_node->m_id == 541094) cout << "checking out leaf 541094 L, h, R " << a_node->m_L << " " << a_node->m_holes << " " << a_node->m_R << endl;
        if(a_node->isRegular()) {
