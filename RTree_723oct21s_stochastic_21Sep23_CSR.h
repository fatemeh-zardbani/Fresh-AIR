...
// copied from 14Sep stochastic version
// Extreme insertion strategy
...
RTREE_TEMPLATE
bool RTREE_QUAL::Search_2027(Node *a_node, Rect *a_rect, int &a_foundCount, LeavesToAdd_v2_ripple *all_lta, int tba_start, int tba_count, vector<overlapping_leaf_tbas> *overlapping_leaves) {
    //            ASSERT(a_node);
    //            ASSERT(a_node->m_level >= 0);
    //            ASSERT(a_rect);
    //            ASSERT(a_node->m_count == a_node->m_branch.size());
...
bool RTREE_QUAL::ripple_lite(const Branch &a_branch, Node *a_node, Node** a_newNode, Rect &a_coverRect, Rect &a_newCoverRect){
    // cout << "in ripple lite  item " << a_branch.m_data << " to leaf " << a_node->m_id << endl;
    if(a_node->m_holes > 0){
        #ifdef stats
            count_total_holes--;
        #endif
    ...
    if(a_node->m_R - a_node->m_L - a_node->m_holes > MOVING_THRESHOLD){
        ...
        // cout << "in mt crack" << endl;
        // too big to move to the back
        // TODO: scrack the leaf, move the smaller piece to the end,
        // choose where to put a_branch and put it there
        // fix the holes of whichever one remains, wether the left or right piece, don't move anymore, just add holes eithr to this or its right brother.
        ...
        // cout << "chosen pivot " << stochastic_crack_pivot << " sci " << stochastic_crack_index << endl;
        ...
        if(stochastic_crack_index != this_L + this_holes && stochastic_crack_index != this_R){
            ...
            if(stochastic_crack_index - this_L - this_holes > this_R - stochastic_crack_index){
                ...
                *a_newNode = AllocNode();
                ...
                (a_node->m_younger_brother)->m_holes += (this_R - stochastic_crack_index);
                (a_node->m_younger_brother)->m_L -= (this_R - stochastic_crack_index);
                #ifdef stats
                ...
                if(a_branch.m_rect.m_max[stochastic_crack_axis] <= stochastic_crack_pivot){
                    ...
                    a_node->m_R++;
                    (a_node->m_younger_brother)->m_holes--;
                    (a_node->m_younger_brother)->m_L++;
                ...
                (*a_newNode)->m_older_brother = m_youngest_leaf;
                m_youngest_leaf->m_younger_brother = (*a_newNode);
                (*a_newNode)->m_younger_brother = NULL;
                m_youngest_leaf = (*a_newNode);
            ...
            // cout << "check in 20" << endl;

            return false;
        }       
    }
    else{
        // cout << "small enogh to mover" << endl;
